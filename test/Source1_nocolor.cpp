...
KinectFusion::KinectFusion() {
	...
	i_rgb.create(cColorHeight, cColorWidth, CV_8UC4);
	i_depth.create(m_cDepthHeight, m_cDepthWidth, CV_8UC1);
	i_depthToRgb.create(m_cDepthHeight, m_cDepthWidth, CV_8SC4);
	m_pNuiSensor = NULL;
	m_pColorFrameReader = NULL;
...
HRESULT KinectFusion::ResetReconstruction()
{
	...
	if (m_bTranslateResetPoseByMinDepthThreshold)
	{
		...
		//worldToVolumeTransform.M33 = 768;   
		worldToVolumeTransform.M33 = 512;	//1m
		//worldToVolumeTransform.M33 = 731;   //0.7m
...
void KinectFusion::update() {
	...
	if (SUCCEEDED(hr)) {
		...
		UINT nColorBufferSize = cColorHeight * cColorWidth * 4;
		if (SUCCEEDED(hr))
		...
		UINT nBufferSize = 0;
		if (SUCCEEDED(hr))
			//hr = pDepthFrame->CopyFrameDataToArray(m_cDepthImagePixels, depthData);
			hr = pDepthFrame->AccessUnderlyingBuffer(&nBufferSize, &depthData);
...
int  main() {
	...
	while (1) {
		cc++;
		kf.update();
		if ((kf.m_bTrackingFailed && kf.m_cLostFrameCounter >= 50 ) || kf.m_cFrameCounter >= 10)
			break;
	}
	cout << "1114" << endl;
	//kf.processResult();
